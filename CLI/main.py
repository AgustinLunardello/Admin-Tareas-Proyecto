...
def delete_task(id_task: int):
    global URL, USERNAME, HEADERS
    r = requests.delete(url=URL+f'/task_delete/{id_task}/{USERNAME}', headers=HEADERS)
...
def update_state_task(id_task: int, new_state: str):
    global URL, USERNAME, HEADERS
    r = requests.put(url=URL+f'/task_new_state/{id_task}/{USERNAME}/{new_state}', headers=HEADERS)
    if r.status_code == 200:
        print(r.json()['message'])
    elif r.status_code == 404:
        print(r.json()['detail'])
